  ...
  return (
    <div className="w-[280px] h-full bg-card/40 backdrop-blur-3xl border-r border-border flex flex-col z-20 relative overflow-hidden">
      ...
      <div className="h-16 px-6 border-b border-border/50 flex items-center justify-between shrink-0 relative z-10">
        <div className="font-black text-[10px] uppercase tracking-[0.25em] text-primary/80 flex items-center gap-2.5">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-4">
        <div>
          <div className="text-[10px] font-bold text-muted-foreground/40 uppercase tracking-widest mb-3 px-3 flex items-center justify-between">
            <span>Library</span>
            ...
            <div className="space-y-1 mt-2">
              {collections.map(col => (
                <div key={col.id} className="text-sm group mb-2">
                  <div className="px-3 py-2 flex items-center justify-between text-foreground font-semibold hover:bg-white/5 rounded-xl cursor-pointer transition-all border border-transparent hover:border-white/5">
                    ...
                    {requests.filter(r => r.collectionId === col.id).map(req => (
                      <div
                        ...
                        className={`group/req px-3 py-2 flex items-center gap-3 rounded-xl cursor-pointer text-[13px] transition-all relative ${activeRequestId === req.id
                            ? 'bg-primary/10 text-primary font-bold shadow-[inset_0_0_20px_rgba(0,174,239,0.05)] border border-primary/20'
